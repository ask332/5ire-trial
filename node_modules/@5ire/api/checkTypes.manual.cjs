...
var _api = require("@5ire/api");

var _testingPairs = require("@polkadot/keyring/testingPairs");

var _create = require("@5ire/types/create");
...
async function query(api, pairs) {
  ...
  console.log('query types:', bar, bal, bal2, override, oldBal, multia, multib); // check multi for unsub
...
async function main() {
  ...
  Promise.all([consts(api), derive(api), errors(api), events(api), query(api, pairs), queryExtra(api, pairs), rpc(api), types(api), tx(api, pairs), at(api)]);
