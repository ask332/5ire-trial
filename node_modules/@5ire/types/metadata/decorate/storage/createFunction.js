...
/** @internal */
export function createKeyRaw(registry, itemFn, keys, hashers, args) {
...
export function createFunction(registry, itemFn, options) {
  ...
  const storageFn = expandWithMeta(itemFn, (...args) => {
    if (type.isPlain) {
      if (!cacheKey) {
        cacheKey = options.skipHashing ? compactAddLength(u8aToU8a(options.key)) : createKey(registry, itemFn, [], [], []);
      }
