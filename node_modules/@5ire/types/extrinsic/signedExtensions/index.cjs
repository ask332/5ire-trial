...
const allExtensions = (0, _util.objectSpread)({}, _substrate.substrate, _polkadot.polkadot, _shell.shell, _statemint.statemint); // the v4 signed extensions prior to the point of exposing these to the metadata.
...
function expandExtensionTypes(extensions, type) {
  ...
  .map(key => userExtensions[key] || allExtensions[key]).filter(info => !!info).reduce((result, info) => (0, _util.objectSpread)(result, info[type]), {});
}