...
function mapCapabilities(_ref, _ref2, _ref3, _ref4) {
  ...
  let [leasePeriodsPerSlot, slotRangeCount] = _ref2;
  let [stakingVersion] = _ref3;
  let [keys, accountInfo] = _ref4;
...
function detectedCapabilities(api, blockHash) {
  ...
  ? (0, _rxjs.of)([]) : (0, _rxjs.of)(consts.filtered) : (0, _rxjs.of)([]), queries.filtered.length ? blockHash ? (0, _rxjs.combineLatest)(queries.filtered.map(c => c.at(blockHash))) : api.queryMulti(queries.filtered) : (0, _rxjs.of)([]), raws.filtered.length ? blockHash ? (0, _rxjs.combineLatest)(raws.filtered.map(k => api.rpc.state.getStorage.raw(k, blockHash))) : (0, _rxjs.combineLatest)(raws.filtered.map(k => api.rpc.state.getStorage.raw(k))) : (0, _rxjs.of)([])]).pipe((0, _rxjs.map)(_ref5 => {
    let [cResults, qResults, rResults] = _ref5;
    ...
    }, extractResults(cResults, consts), extractResults(qResults, queries), extractResults(rResults.map(r => api.registry.createType('Raw', r)), raws));
  }), (0, _rxjs.take)(1), (0, _rxjs.catchError)(() => (0, _rxjs.of)({})));
}