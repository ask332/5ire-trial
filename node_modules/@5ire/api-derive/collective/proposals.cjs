...
function _proposalsFrom(instanceId, api, section) {
  return (0, _index.memo)(instanceId, hashes => {
    ...
    (0, _rxjs.combineLatest)(hashes.map(hash => api.query[section].proposalOf(hash).pipe((0, _rxjs.catchError)(() => (0, _rxjs.of)(null))))), api.query[section].voting.multi(hashes)]) : (0, _rxjs.of)([[], [], []])).pipe((0, _rxjs.map)(result => parse(api, result)));
  });
}
