...
function _proposalsFrom(instanceId, api, section) {
  return memo(instanceId, hashes => {
    ...
    combineLatest(hashes.map(hash => api.query[section].proposalOf(hash).pipe(catchError(() => of(null))))), api.query[section].voting.multi(hashes)]) : of([[], [], []])).pipe(map(result => parse(api, result)));
  });
}
