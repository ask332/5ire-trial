...
export const allExtensions = objectSpread({}, substrate, polkadot, shell, statemint); // the v4 signed extensions prior to the point of exposing these to the metadata.
...
export function expandExtensionTypes(extensions, type, userExtensions = {}) {
  ...
  .map(key => userExtensions[key] || allExtensions[key]).filter(info => !!info).reduce((result, info) => objectSpread(result, info[type]), {});
}