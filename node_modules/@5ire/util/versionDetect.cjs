...
function detectPackage(_ref3, pathOrFn) {
  ...
  (0, _assert.assert)(name.startsWith('@polkadot') || name.startsWith('@5ire'), () => `Invalid package descriptor ${name}`);
  ...
  if (entry.length !== 1) {
    ...
    const mismatches = deps.filter(d => {
      ...
      const isDep5irePackage = d.name.startsWith('@5ire'); // Only compare same package names
